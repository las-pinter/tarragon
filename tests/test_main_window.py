...
class TestInstantiation:
    """MainWindow construction and basic structure."""

    def test_main_window_is_qmainwindow(self) -> None:
...
class TestDockWidgets:
    """MainWindow creates the sidebar, grid, and preview docks."""

    def test_three_docks_exist(self, qapp: Any, mock_settings: MagicMock) -> None:
    ...
    def test_docks_have_correct_titles(self, qapp: Any, mock_settings: MagicMock) -> None:
    ...
    def test_docks_are_attached_to_window(self, qapp: Any, mock_settings: MagicMock) -> None:
...
class TestMenuActions:
    """MainWindow menu bar actions are registered and wired."""

    def test_open_folder_menu_exists(self, qapp: Any, mock_settings: MagicMock) -> None:
...
class TestDefaultSize:
    """MainWindow opens at the default window size."""
...
class TestFilteredQueryRegression:
    ...
    def test_run_filtered_query_does_not_clear_gallery_when_no_folder(
        ...
        """The _run_filtered_query() method does not clear the model when _current_folder is empty."""
        window = MainWindow(settings_service=mock_settings)
        try:
    ...
    def test_run_filtered_query_works_when_folder_is_set(
        ...
        """The _run_filtered_query() method queries the DB and updates the model when a folder is set."""
        window = MainWindow(settings_service=mock_settings)
        try:
            db = Database(Path(":memory:"))
            db.init_schema()
    ...
    def test_has_filters_includes_tag_filters(
        ...
        """The _on_open_folder() method detects active tag filters and runs a filtered query."""
        window = MainWindow(settings_service=mock_settings)
        try:
            db = Database(Path(":memory:"))
            db.init_schema()
    ...
    def test_filtered_query_returns_empty_when_no_match(
        ...
        """Filtered query returns 0 results without fallback when nothing matches."""
        window = MainWindow(settings_service=mock_settings)
        try:
            db = Database(Path(":memory:"))
            db.init_schema()
...
class TestFolderFiltering:
    """Folder filters restrict results in global mode."""
